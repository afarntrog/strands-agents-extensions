...
def manager():
    """Create in-memory session manager for testing with clean database."""
    # Use a different session_id for the manager itself to avoid conflicts with test sessions
    mgr = SQLiteSessionManager(session_id="manager-session", db_path=":memory:")
    yield mgr
    if mgr._conn:
        mgr._conn.close()
...
class TestAgentOperations:
    ...
    def test_cascade_delete_agents(self, manager, session, agent):
        ...
        result = manager.read_agent(session.session_id, agent.agent_id)
        assert result is None
